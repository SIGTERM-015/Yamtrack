...
class CompleteMediaSerializer(serializers.Serializer):
    ...
    def _move_extra_keys_into_details(self, media_metadata, details):
        """Move top-level extra keys into the details dict."""
    ...
    def _coerce_detail_types(self, details):
        """Coerce detail values from provider strings to response types."""
        if "year" in details:
    ...
    def _normalize_details(self, media_metadata):
        """Move extra keys into details and coerce detail value types."""
        details = media_metadata.get("details", {})
        self._move_extra_keys_into_details(media_metadata, details)
        self._coerce_detail_types(details)
        return details
    ...
    def to_representation(self, instance):
        ...
        details = self._normalize_details(media_metadata)
...
def _instantiate(serializer_class, data, *, many, context):
    """Instantiate a serializer with optional context and return its data."""
    kwargs = {"many": many}
...
def _missing_serializer_error(data_type):
    """Build the error for a data type with no registered serializer."""
    return ValueError(
        f"No serializer found for data type {data_type}. "
        f"Supported types: {list(serializer_map.keys())}. "
        "Pass serializer_class explicitly if needed.",
    )


def _serialize_many(data_list, *, context, homogeneous):
    """Serialize a list using auto-detected or mixed serializer."""
    if homogeneous:
        first_type = type(data_list[0])
        detected_serializer_class = serializer_map.get(first_type)
        if detected_serializer_class is None:
            raise _missing_serializer_error(first_type)
        return _instantiate(
            detected_serializer_class,
            data_list,
            many=True,
            context=context,
        )
    return _instantiate(MixedMediaSerializer, data_list, many=True, context=context)
...
def serialize_data(
    ...
    *,
    ...
    if serializer_class is not None:
        return _instantiate(serializer_class, data, many=many, context=context)
    ...
    if many:
        ...
        if not data_list:
            return []
        return _serialize_many(data_list, context=context, homogeneous=homogeneous)

    data_type = type(data)
    detected_serializer_class = serializer_map.get(data_type)
    if detected_serializer_class is None:
        raise _missing_serializer_error(data_type)
    return _instantiate(
        detected_serializer_class,
        data,
        many=False,
