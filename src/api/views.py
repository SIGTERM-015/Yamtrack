...
# TODO: Implement search for already tracked media (item_id and tracked fields)  # noqa: E501, W505

# TODO: Implement global search endpoint for every media_type

# TODO: Implement admin commands to manage users (add admins, remove/add users, etc)  # noqa: E501, W505

# TODO: Move operations on db to `models` file of the relative django app  # noqa: E501, W505

# TODO!!: since it's possible to add to lists untracked items, the id field can be null, so it's impossible to get these elements from the list, while it should be possible. The untracked added element is in the Items table, but not in the media tables. Add the list of lists an item is in to the model of the medias, so they can be retrieved and computed easily.  # noqa: E501, W505

# TODO: look into django.core.paginator Paginator

# TODO: Review children endpoints performance and avoid repeated list lookups per item.  # noqa: E501, W505


def _item_display_name(cleaned_data):
    """Build a human-readable name for a manually tracked item."""
    name = cleaned_data.get("title", "item")
    if cleaned_data.get("season_number"):
        name += f" - Season {cleaned_data['season_number']}"
    if cleaned_data.get("episode_number"):
        name += f" - Episode {cleaned_data['episode_number']}"
    return name


def _strip_recommendations(media_metadata):
    """Remove the recommendations entry from media metadata related data."""
    related = media_metadata.get("related")
    if related is not None:
        related.pop("recommendations", None)
    return media_metadata


def _get_related_seasons(media_metadata):
    """Return the seasons list from media metadata related data, if present."""
    related = media_metadata.get("related")
    if related is None:
        return []
    return related.get("seasons", [])


def _season_lists_by_number(user, media_id, source, results):
    """Map each season number to the lists the user has for that season."""
    lists_by_number = {}
...
def _tracked_seasons_by_number(user, media_id, source, season_numbers):
    """Map each season number to the user's tracked season for it, if any."""
    tracked_by_number = {}
    if not season_numbers:
        return tracked_by_number
...
def _temp_season_entry(item, lists):
    """Build a lightweight season entry for an untracked season."""
    return type(
        "TempMedia",
        (),
        {
...
def _build_season_media_entries(user, media_id, source, results):
    """Build serializable season entries for the given season metadata results."""
    lists_by_number = _season_lists_by_number(user, media_id, source, results)
    ...
    tracked_by_number = _tracked_seasons_by_number(
        user,
        media_id,
        source,
        season_numbers,
    ...
    for season in results:
        ...
        entries.append(_temp_season_entry(item, lists))

    return entries


def _check_season_endpoint(
    media_type,
    source,
    tv_only_detail,
    query_detail,
    ...
    """Return an error Response for a media type/source combo that cannot be queried.

    ``tv_only_detail`` and ``query_detail`` are the exact detail strings the
    original endpoint view must surface. ``unsupported_detail`` overrides the
    default unsupported media type message when a view needs a custom one.
    """
    if not check_valid_type(media_type):
        return Response(
            {"detail": unsupported_detail or "Unsupported media type."},
            status=HTTP.BAD_REQUEST,
        )

    if media_type != MediaTypes.TV.value:
        return Response(
            {"detail": tv_only_detail},
            status=HTTP.BAD_REQUEST,
        )
...
def _check_media_endpoint(media_type, source):
    """Return an error Response for a media type/source combo that cannot be queried."""
    if not check_valid_type(media_type):
        return Response(
...
def _save_refresh_and_fetch_metadata(
    media,
    metadata_args,
    save_detail,
    metadata_detail,
    *,
    ...
    """Save a tracked item and return its refreshed metadata.

    Returns ``(media_metadata, episode, error)``. ``error`` is a Response when
    the save failed, the metadata could not be fetched, or the requested item
    is missing.
    """
    try:
        media.save()
    except Exception as e:  # noqa: BLE001
        body = {"detail": save_detail}
        if save_errors:
            body["errors"] = str(e)
        return None, None, Response(body, status=HTTP.BAD_REQUEST)

    media.refresh_from_db()

    try:
        media_metadata = services.get_media_metadata(*metadata_args)
    except Exception as e:  # noqa: BLE001
        return (
            None,
            None,
            Response(
                {"detail": metadata_detail, "errors": str(e)},
                status=HTTP.INTERNAL_SERVER_ERROR,
            ),
        )

    if not_found_detail is not None and not media_metadata:
        return (
            None,
            None,
            Response(
    ...
    if episode_number is not None:
        has_episodes, episode = _find_episode_entry(
            media_metadata,
            episode_number,
        )
        if has_episodes and not episode:
            return (
                None,
                None,
                Response(
    ...
    return media_metadata, episode, None


def _apply_validated_updates(media, body, item_kind):
    """Validate a patch body and apply the accepted fields to a tracked item.

    Returns the validation error, or None when every field was applied.
    """
    validated_body, error = validate_body(body, item_kind)
    if error:
        return error
...
def _find_episode_entry(media_metadata, episode_number):
    """Return (has_episodes, entry) for an episode number in season metadata.

    has_episodes is False when the metadata carries no episode list; callers
    must not treat that as a missing episode.
    """
    episodes = media_metadata.get("episodes")
    if episodes is None:
        return False, None

    return True, next(
        (obj for obj in episodes if obj["episode_number"] == int(episode_number)),
        None,
    )


def _episode_lists_by_number(user, media_id, source, season_number, results):
    """Map each episode number to the lists the user has for that episode."""
    lists_by_number = {}
...
def _tracked_episodes_by_number(user, media_id, source, season_number, episode_numbers):
    """Map each episode number to the user's tracked episode for it, if any."""
    tracked_by_number = {}
...
class MediaTypeListView(drf_views.APIView):
    ...
    def post(self, request, media_type):
        ...
        if body.get("source", Sources.MANUAL.value) == Sources.MANUAL.value:
            return self._track_manual_item(request, body)

        return self._track_provider_item(request, body, media_type)

    def _track_manual_item(self, request, body):
        """Create a manually tracked media item from the request body."""
        ...
        try:
            ...
            name = _item_display_name(form.cleaned_data)
            return Response(
                {"detail": f"Conflict. {name} already exists."},
    ...
    def _track_provider_item(self, request, body, media_type):
        """Create a provider tracked item, fetching its metadata first."""
...
class MediaDetailView(drf_views.APIView):
    ...
    def delete(self, request, media_type, source, media_id):
        ...
        if error := _check_media_endpoint(media_type, source):
            return error
    ...
    def get(self, request, media_type, source, media_id):
        ...
        media_metadata = _strip_recommendations(media_metadata)
    ...
    def patch(self, request, media_type, source, media_id):
        ...
        media_metadata, _, error = _save_refresh_and_fetch_metadata(
            media,
...
class MediaSeasonsView(drf_views.APIView):
    ...
    def get(self, request, media_type, source, media_id):
        ...
        paginated_data = paginate_data(
            request, _get_related_seasons(media_metadata), limit, offset
        )
        season_media_entries = _build_season_media_entries(
            user,
            media_id,
            source,
            paginated_data["results"],
...
class MediaSeasonDetailView(drf_views.APIView):
    ...
    def delete(self, request, media_type, source, media_id, season_number):
        ...
        if error := _check_season_endpoint(
            media_type,
            source,
    ...
    def patch(self, request, media_type, source, media_id, season_number):
        ...
        error = _apply_validated_updates(media, body, "season")
        if error:
            return Response(
                {"detail": f"{error}"},
                status=HTTP.BAD_REQUEST,
        ...
        media_metadata, _, error = _save_refresh_and_fetch_metadata(
            ...
            HTTP.INTERNAL_SERVER_ERROR.phrase,
        )
        if error:
            return error
...
class MediaEpisodeDetailView(drf_views.APIView):
    ...
    def patch(
        ...
        error = _apply_validated_updates(media, body, "episode")
        if error:
            return Response(
                {"detail": HTTP.BAD_REQUEST.phrase, "errors": str(error)},
                status=HTTP.BAD_REQUEST,
        ...
        media_metadata, episode, error = _save_refresh_and_fetch_metadata(
            media,
            ("season", media_id, source, [season_number]),
            HTTP.BAD_REQUEST.phrase,
            HTTP.INTERNAL_SERVER_ERROR.phrase,
            save_errors=True,
            not_found_detail="Episode not found.",
            episode_number=episode_number,
        )
        if error:
            return error
...
class MediaEpisodeListsView(drf_views.APIView):
    ...
    def get(self, request, media_type, source, media_id, season_number, episode_number):
        ...
        if error := _check_season_endpoint(
            ...
            "Seasons are supported only for 'tv' media type.",
            f"Cannot query `{source}` for `{media_type}` media type",
            unsupported_detail=" Unsupported media type.",
        ):
            return error
