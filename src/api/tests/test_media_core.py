...
#: Titles of the TMDB-sourced TV fixtures listed in the "related" payloads.
HOTARU_NO_HIKARI_TITLE = "Hotaru no Hikari: It's Only a Little Light in My Life"
SEVENTH_TIME_LOOP_TITLE = (
...
class MediaCoreTests(YamtrackApiTestCase):
    ...
    def test_media_consumption_entry_detail_invalid_consumption_id_methods(self):
        """Entry-detail endpoints return not found for unknown consumption ids."""
        movie_item = self.items_by_type[MediaTypes.MOVIE.value][0]
    ...
    def test_media_lists_get_invalid_media_id_returns_empty_results(self):
        """Media list relation endpoint returns empty results for unknown media."""
        response = self.call_api(
            "get",
            "api_media_lists",
            args=(MediaTypes.MOVIE.value, "tmdb", 999999),
